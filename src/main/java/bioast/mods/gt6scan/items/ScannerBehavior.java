...
public class ScannerBehavior extends IBehavior.AbstractBehaviorDefault implements IItemGuiHolder {
	...
	private IWidget wItem(OreDictMaterial mat,Mode mode) {
		...
		if (mode == Mode.ROCK) prefix = OP.rockGt;
	...
	public ModularScreen createGuiScreen(EntityPlayer player, ItemStack itemStack) {
		...
		return ModularScreen.simple("map", guiContext -> {
			...
			panel.flex().align(Alignment.Center).size(300, 166).flex().full();
		...
		if (tMap != null) {
			try {
				...
				tTileMap.forEach((chunkPos, tile) -> {
					if (tile instanceof PrefixBlockTileEntity pTile && (mode == Mode.LARGE || mode == Mode.SMALL || mode == Mode.BEDROCK)) {
					...
					if (mode == Mode.FLUID && tile instanceof MultiTileEntityFluidSpring) {
					...
					if (tile instanceof MultiTileEntityRock && mode == Mode.ROCK) {
